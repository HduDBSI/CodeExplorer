...
def test(testlist, model_index, ramData, batch_size, test_id):
    # 定义输出文件路径
    output_file = str(test_id)+".txt"
    ...
    with open(output_file, 'w') as f:
        for i in Test_data_batches:
            ...
            for codepair in batches[i]:
                ...
                h2_batch.append(h2)
                label_batch.append(label)
                codepair_batch.append(codepair)
...
y_preds = test(testlist, model_index, ramData, batch_size, test_id)

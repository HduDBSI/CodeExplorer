package com.hdu.edu.codeexplorer.controller;
import com.hdu.edu.codeexplorer.entity.TestList;
import com.hdu.edu.codeexplorer.service.TestListService;
import com.hdu.edu.codeexplorer.util.ResponseUtil;
import org.apache.logging.log4j.LogManager;
import org.apache.logging.log4j.Logger;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.transaction.annotation.EnableTransactionManagement;
import org.springframework.web.bind.annotation.*;

import java.util.List;

@CrossOrigin
@RestController
@EnableTransactionManagement
@RequestMapping("/test")
public class TestListController {

    @Autowired
    TestListService testListService;

    private static Logger logger = LogManager.getLogger(LogManager.ROOT_LOGGER_NAME);

    // 1. 查询所有任务信息
    @GetMapping("/getAllTest")
    public Object getAllTestLists() {
        try {
            List<TestList> testList = testListService.getAllTestLists();
            return ResponseUtil.okList(testList);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("操作失败，获取所有任务信息失败：" + e);
            return ResponseUtil.fail(-1, "获取所有任务信息失败：" + e.getMessage());
        }
    }

    // 2. 增加任务信息
    @PostMapping("/addTest")
    public Object addTestList(@RequestBody TestList testList) {
        try {
            int result = testListService.addTestList(testList);
            if (result > 0) {
                return ResponseUtil.ok();
            } else {
                return ResponseUtil.fail(-1, "增加任务失败");
            }
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("操作失败，增加任务失败：" + e);
            return ResponseUtil.fail(-1, "增加任务失败：" + e.getMessage());
        }
    }

    // 3. 根据ID删除任务信息
    @DeleteMapping("/deleteTest/{id}")
    public Object deleteTestListById(@PathVariable("id") Integer id) {
        try {
            int result = testListService.deleteTestListById(id);
            if (result > 0) {
                return ResponseUtil.ok();
            } else {
                return ResponseUtil.fail(-1, "任务ID不存在，删除失败");
            }
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("操作失败，根据ID删除任务失败：" + e);
            return ResponseUtil.fail(-1, "删除任务失败：" + e.getMessage());
        }
    }

    // 4. 修改任务信息
    @PutMapping("/updateTest")
    public Object updateTestListById(@RequestBody TestList testList) {
        try {
            int result = testListService.updateTestListById(testList);
            if (result > 0) {
                return ResponseUtil.ok();
            } else {
                return ResponseUtil.fail(-1, "更新任务失败");
            }
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("操作失败，更新任务失败：" + e);
            return ResponseUtil.fail(-1, "更新任务失败：" + e.getMessage());
        }
    }

    // 5. 根据ID查询任务信息
    @GetMapping("/getTestById/{id}")
    public Object getTestById(@PathVariable("id") Integer id) {
        try {
            TestList test = testListService.getTestById(id);
            if (test != null) {
                return ResponseUtil.ok(test);
            } else {
                return ResponseUtil.fail(-1, "任务ID不存在");
            }
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("操作失败，根据ID获取任务信息失败：" + e);
            return ResponseUtil.fail(-1, "获取任务信息失败：" + e.getMessage());
        }
    }
}
